...
switch (_code) do {
		...
		if(_alt && !_shift) then 
	{
		...
		diag_log format [" %1 ALT+F4 Cekmeye Calisti! (If it made after the reported admin.)",_player getVariable["realname",name _player]];
		[[1 format [ "% Calisti to 1 ALT + F4 Czech! (With the admin is done after the incident.)" _ Player getvariabl the [ "realname" name is _play]]], "life_fnc_broadcast", nil, false] spawn life_fnc_mp;
		SOCK_fnc_updateRequest;
		};
	};
	...
	{
		SOCK_fnc_updateRequest;
		diag_log format [ "% 1 ALT + F4 to Calisti Czech! (if made after she reported to the admin.)" _ player getvariabl the [ "realname" name is _play]];
		[[1 format [ "% Calisti to 1 ALT + F4 Czech! (With the admin is done after the incident.)" _ Player getvariabl the [ "realname" name is _play]]], "life_fnc_broadcast", nil, false] spawn life_fnc_mp;
		SOCK_fnc_updateRequest;
		};
	};
	...
	//E
	case 18:	{	
	if (!_alt && !_ctrlKey && playerSide == west && _shift) then		{			[] call life_fnc_radar;		};	
	};
