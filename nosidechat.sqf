[] spawn {	disableSerialization;
		...
		if (disconnect_me == 0) then { 
			if (isNil "warn_one") then { 
				warn_one = true;
				systemChat ("Stop using global chat channels, This is your first warning");
				[] spawn DS_slap_them;	
				playSound "flashbang";	
		...
		if (disconnect_me == 1) then {				
			if (isNil "warn_two") then {		
				warn_two = true;	
				systemChat ("Stop using global chat channels, This is your second warning..");	
				[] spawn DS_slap_them;			
				playSound "flashbang";		
				["NO VOICE ON SIDE"] spawn DS_double_cut;
			};		
		...
		if (disconnect_me == 2) then {		
			if (isNil "warn_three") then {	
				warn_three = true;			
				systemChat ("Stop using global chat channels, This is your LAST warning!");	
				systemChat ("You will be frozen and then disconnected!");		
				[] spawn DS_slap_them;			
				playSound "flashbang";		
				["Stop using global chat channels, This is your LAST warning!"] spawn DS_double_cut;	
			};			
		};			
